...
Enhanced with Detection Snapshots:
- Saves snapshot images when objects (House, Tank, Tree) are detected
- Displays detection location (X, Y) and orientation on GUI
- Shows detection history with images in scrollable panel

GUI version with tkinter

FIXED: Wall detection now works correctly for all drone orientations (North, East, South, West)
"""
...
detected_snapshots = []  # List of detection snapshots: [(target_num, object_name, confidence, location, orientation, snapshot_path, timestamp), ...]
...
class Challenge2GUI:
    def __init__(self, root):
        ...
        self.root.geometry("1600x900")  # Increased width for detection history panel
        ...
        # Detection history widgets
        self.detection_history_widgets = []
    ...
    def create_gui(self):
        ...
        main_frame.rowconfigure(1, weight=0)  # Log row at bottom
        ...
        # Middle panel - Maze Visualization
        viz_frame = ttk.LabelFrame(main_frame, text="Maze Visualization", padding="5")
        viz_frame.grid(row=0, column=1, sticky="nsew", padx=5)
        viz_frame.columnconfigure(0, weight=1)
        viz_frame.rowconfigure(0, weight=1)

        self.fig, self.ax = plt.subplots(figsize=(6, 6))  # Square figure
        self.canvas = FigureCanvasTkAgg(self.fig, master=viz_frame)
        self.canvas.get_tk_widget().grid(row=0, column=0, sticky="nsew")

        # Right panel - Object Detection View
        detection_frame = ttk.LabelFrame(main_frame, text="Live Object Detection", padding="5")
        detection_frame.grid(row=0, column=2, sticky="nsew", padx=5)
        detection_frame.columnconfigure(0, weight=1)
        detection_frame.rowconfigure(0, weight=1)

        # Create rectangular canvas for detection image (640x480 for drone camera)
        self.detection_canvas = tk.Canvas(detection_frame, width=480, height=360, bg='black')
        ...
        # Far Right panel - Detection History (Scrollable)
        history_container = ttk.LabelFrame(main_frame, text="Detection History", padding="5")
        history_container.grid(row=0, column=3, sticky="nsew", padx=5)

        # Create canvas and scrollbar for history panel
        history_canvas = tk.Canvas(history_container, width=350, highlightthickness=0)
        history_scrollbar = ttk.Scrollbar(history_container, orient="vertical", command=history_canvas.yview)
        self.history_panel = ttk.Frame(history_canvas)

        self.history_panel.bind(
            "<Configure>",
            lambda e: history_canvas.configure(scrollregion=history_canvas.bbox("all"))
        )

        history_canvas.create_window((0, 0), window=self.history_panel, anchor="nw")
        ...
        history_canvas.bind_all("<MouseWheel>", _on_history_mousewheel)

        # Button to clear detection history
        clear_history_btn = ttk.Button(history_container, text="Clear History",
                                       command=self.clear_detection_history)
        clear_history_btn.pack(side="bottom", pady=5)

        # Log Frame (spans all columns at the bottom)
        log_frame = ttk.LabelFrame(main_frame, text="Log", padding="5")
        log_frame.grid(row=1, column=0, columnspan=4, sticky="nsew", pady=(5, 0))
        ...
        # Targets listbox
        self.race_targets_listbox = tk.Listbox(race_targets_frame, height=5, width=23)
    ...
    def initialize_video_detection(self):
        ...
        try:
            ...
            self.log("Expecting objects: House, Tank, Tree")
        except Exception as e:
    ...
    def clear_detection_history(self):
        """Clear all detection history widgets and global list"""
        global detected_snapshots

        # Clear widgets
        for widget in self.detection_history_widgets:
            widget.destroy()
        self.detection_history_widgets = []
        ...
        self.log("Detection history cleared")

    def save_detection_snapshot(self, frame, obj_name, confidence, target_num, location, orientation):
        """Save detection snapshot to file and return filepath"""
        global detected_snapshots
        ...
        try:
            ...
            # Create filename
            filename = f"T{target_num}_{obj_name}_{timestamp}.jpg"
            filepath = os.path.join(SNAPSHOTS_DIR, filename)

            # Save image
            cv2.imwrite(filepath, frame)

            # Add to global snapshots list with all metadata
            snapshot_data = {
                'target_num': target_num,
                'object_name': obj_name,
                'confidence': confidence,
                'location': location,  # (x, y) tuple
                'orientation': orientation,  # "North", "East", etc.
                'filepath': filepath,
                'timestamp': timestamp
            }
            detected_snapshots.append(snapshot_data)

            self.log(f"[SNAPSHOT] Saved: {filename}")
            self.log(f"  Location: ({location[0]}, {location[1]}) | Orientation: {orientation}")

            return filepath

        except Exception as e:
            self.log(f"[X] Error saving snapshot: {e}")
            return None

    def add_detection_to_history(self, snapshot_data):
        """Add a detection snapshot to the history panel (thread-safe)"""

        def _add():
            try:
                # Create frame for this detection
                det_frame = ttk.Frame(self.history_panel, relief="solid", borderwidth=2, padding=5)
                det_frame.pack(fill="x", padx=5, pady=5)

                # Load and display thumbnail
                img = Image.open(snapshot_data['filepath'])
                img.thumbnail((300, 225))  # Maintain aspect ratio
                photo = ImageTk.PhotoImage(img)

                img_label = tk.Label(det_frame, image=photo)
                img_label.image = photo  # Keep reference
                img_label.pack()

                # Detection info
                info_text = f"Target {snapshot_data['target_num']}: {snapshot_data['object_name']}\n"
                info_text += f"Confidence: {snapshot_data['confidence']:.1%}\n"
                info_text += f"Location: ({snapshot_data['location'][0]}, {snapshot_data['location'][1]})\n"
                info_text += f"Orientation: {snapshot_data['orientation']}\n"
                info_text += f"Time: {snapshot_data['timestamp']}"

                info_label = ttk.Label(det_frame, text=info_text,
                                       font=('Arial', 9), justify='left',
                                       foreground='darkblue')
                info_label.pack(pady=5)

                # Separator
                ttk.Separator(det_frame, orient='horizontal').pack(fill='x', pady=5)

                # Store widget reference
                self.detection_history_widgets.append(det_frame)

            except Exception as e:
                self.log(f"[X] Error adding to history panel: {e}")

        # Thread-safe GUI update
        self.root.after(0, _add)

    def update_live_detection_feed(self, frame):
        """Update the detection canvas with live video feed (thread-safe)"""
        try:
            # Convert BGR to RGB
            frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

            # Resize frame to fit rectangular canvas (480x360)
            height, width = frame_rgb.shape[:2]
            target_width = 480
            target_height = 360

            # Calculate scaling to fit (maintain aspect ratio, fill the canvas)
            ...
            # Crop to fit the canvas exactly (center crop if needed)
            if new_width > target_width:
                start_x = (new_width - target_width) // 2
                resized_frame = resized_frame[:, start_x:start_x + target_width]
            if new_height > target_height:
                start_y = (new_height - target_height) // 2
                resized_frame = resized_frame[start_y:start_y + target_height, :]
            ...
            def _update_display():
                ...
                self.detection_photo = photo

                # Display on canvas
                self.detection_canvas.delete("all")

                # Display at origin (no offset needed since we cropped to exact size)
                self.detection_canvas.create_image(0, 0,
                                                   anchor=tk.NW,
                                                   image=photo)
            ...
            # Silently fail for live feed updates to avoid log spam
            pass
    ...
    def display_detection_image(self, frame, obj_name, confidence, target_num):
        ...
        try:
            ...
            # Crop to fit the canvas exactly
            if new_width > target_width:
                start_x = (new_width - target_width) // 2
                resized_frame = resized_frame[:, start_x:start_x + target_width]
            if new_height > target_height:
    ...
    def rotate_to_facing(self, target_facing):
        ...
        if turn != 0:
            ...
            time.sleep(0.2)  # Wait for rotation to complete
            self.current_facing = target_facing

    def detect_object_at_target(self, target_num, current_location, current_orientation, is_first_target=False):
        """Hover at target and detect object using configurable hover time

        Args:
            target_num: Target number for identification
            current_location: Tuple (x, y) of current cell position
            current_orientation: String indicating drone facing direction
            is_first_target: Boolean indicating if this is the first target (need to start video)
        """
        ...
        self.log(f"=" * 60)
        self.log(f"TARGET {target_num} - OBJECT DETECTION")
        self.log(f"Location: ({current_location[0]}, {current_location[1]})")
        self.log(f"Orientation: {current_orientation}")
        self.log(f"Hovering for {hover_time} seconds...")
        self.log(f"=" * 60)

        try:
            # Only start video mode on first target, keep it running for subsequent targets
            if is_first_target:
                self.log("Starting video stream (first target)...")
                vid.video_mode_on()
                time.sleep(0.1)  # Give video mode time to initialize

                self.log("Video mode on - starting recording...")
                vid.startrecording()
                time.sleep(0.2)  # Give recording time to start
            else:
                self.log("Video already streaming - continuing detection...")
                # Video is already running from previous target
                time.sleep(0.1)  # Brief pause before starting detection

            self.log("Beginning detection loop...")
            self.log("Looking for: House, Tank, or Tree")

            detected_obj = None
            detected_confidence = 0.0
            detected_frame = None
            detection_count = 0
            detection_threshold = 1  # Need 1 consecutive detections
            ...
            while time.time() - start_time < hover_time:  # Configurable detection window
                try:
                    ...
                    # Get detection result - IMPORTANT: detector returns (obj_info, annotated_frame)
                    ...
                    # Update live feed in GUI
                    self.update_live_detection_feed(annotated_frame)
                    ...
                    if obj_found is not None:
                        # Debug: Log the type and content of detection on first occurrence
                        if frame_count == 1:
                            self.log(f"[DEBUG] Detector response type: {type(obj_found)}")
                            self.log(f"[DEBUG] Detector response: {obj_found}")

                        # Handle dictionary response from detector
                        if isinstance(obj_found, dict):
                            obj_name = obj_found.get('label', None)
                            confidence = obj_found.get('score', 0.0)
                        # Handle string response
                        elif isinstance(obj_found, str):
                            obj_name = obj_found
                            confidence = 0.8  # Default confidence
                        else:
                            obj_name = None

                    if obj_name is not None and obj_name in ['House', 'Tank', 'Tree']:
                        # Count consecutive detections of same object
                        if obj_name == last_detected:
                            detection_count += 1
                        else:
                            detection_count = 1
                            last_detected = obj_name
                            self.log(f"Detected: {obj_name} (conf: {confidence:.2%})")
                        ...
                        if detection_count >= detection_threshold and detected_obj is None:
                            ...
                            self.log("=" * 60)
                            self.log(f"✓ CONFIRMED DETECTION: {detected_obj}")
                            self.log(f"  Confidence: {detected_confidence:.2%}")
                            self.log(f"  Location: ({current_location[0]}, {current_location[1]})")
                            self.log(f"  Orientation: {current_orientation}")
                            self.log("=" * 60)
                            break
                        ...
                        if frame_count % 20 == 0:
                            self.log(f"Scanning... ({int(time.time() - start_time)}s elapsed)")
                        detection_count = 0
                        last_detected = None
            ...
            # Don't stop recording - keep video running for next target
            # Video will be stopped at the end of the race

            # Log result
            elapsed = time.time() - start_time
            if detected_obj:
                self.log(f"[✓] Detection successful in {elapsed:.1f}s")
                detected_objects.append((target_num, detected_obj, detected_confidence))

                # Save snapshot with location and orientation
                if detected_frame is not None:
                    snapshot_path = self.save_detection_snapshot(
                        detected_frame.copy(),
                        detected_obj,
                        detected_confidence,
                        target_num,
                    ...
                    # Display detection image in live view
                    ...
                    # Add to detection history panel
                    if snapshot_path:
                        snapshot_data = detected_snapshots[-1]  # Get the most recently added snapshot
                        self.add_detection_to_history(snapshot_data)
            else:
            ...
            # Don't stop recording here - let the race loop handle it
            return None
    ...
    def run_complete_mapping(self):
        ...
        try:
            ...
            def get_walls_from_obstacles(obs, cx, cy):
                """Convert obstacle readings to walls based on drone's current facing direction"""
                walls = []

                # Transform obstacle directions based on drone's facing
                # Mapping: obstacle sensor -> absolute direction based on facing
                direction_map = {
                    "North": {
                        'forward': (0, 1),   # +Y
                        'back': (0, -1),     # -Y
                ...
                transforms = direction_map[self.current_facing]

                for sensor, detected in obs.items():
                    if detected and sensor in transforms:
                        dx, dy = transforms[sensor]
                        nx, ny = cx + dx, cy + dy

                        # Check bounds
                        if 0 <= nx < cols and 0 <= ny < rows:
                            wall = tuple(sorted([(cx, cy), (nx, ny)]))
                            walls.append(wall)
            ...
            self.log(f"Initial drone facing: {self.current_facing}")
    ...
    def start_race(self):
        ...
        # Clear detection history at start of new race
    ...
    def run_race(self):
        ...
        try:
            ...
            targets = self.race_targets.copy()
            ...
            for target_idx, target_info in enumerate(targets):
                ...
                if self.enable_detection.get():
                    ...
                    # Pass is_first_target flag
                    is_first = (target_idx == 0)
                    detected_obj = self.detect_object_at_target(
                        target_num,
                        current_pos,  # (x, y)
                        self.current_facing,  # orientation
                        is_first_target=is_first
                    )
                    time.sleep(0.2)
                else:
            ...
            # Stop video recording after all targets
            if self.enable_detection.get():
                try:
                    self.log("Stopping video recording...")
                    if vid is not None:
                        vid.stoprecording()
                        self.log("[OK] Video stopped")
                except Exception as e:
            ...
            if detected_objects:
                self.log("=" * 60)
                self.log("DETECTED OBJECTS SUMMARY")
                self.log("=" * 60)
            ...
            # Save race results with detection snapshots
            ...
            # Add snapshot data to race results
            snapshots_json = []
            for snapshot in detected_snapshots:
                snapshots_json.append({
                    "target": snapshot['target_num'],
                    "object": snapshot['object_name'],
                    "confidence": float(snapshot['confidence']),
                    "location": {
                        "x": snapshot['location'][0],
                        "y": snapshot['location'][1]
                    },
            ...
            race_results = {
                ...
                "detection_snapshots": snapshots_json,
            ...
            self.log(f"[OK] {len(detected_snapshots)} detection snapshots saved in '{SNAPSHOTS_DIR}/'")
            ...
            try:
                if vid is not None:
                    self.log("Cleaning up video stream...")
                    vid.stoprecording()
                    vid.close()
            except Exception as cleanup_error:
                self.log(f"[!] Error during video cleanup: {cleanup_error}")
