...
class Maze:
    def __init__(self, size=5):
        self.size = size
        # Initialize all walls as unknown (None) instead of True
        self.h_walls = [[None] * size for _ in range(size + 1)]
        self.v_walls = [[None] * (size + 1) for _ in range(size)]
        self.start = None
        self.target = None
        
    def set_start_target(self, start, target):
        """Manually set start and target positions"""
        self.start = start
        self.target = target
    
    def update_walls_from_sensors(self, pos, sensor_data):
        """Update maze walls based on real sensor readings from pyhula"""
        r, c = pos
        
        # Update horizontal walls based on forward/backward sensors
        if sensor_data.get("forward") is not None:
            if r > 0:
                self.h_walls[r][c] = sensor_data["forward"]  # True if wall exists
        
        if sensor_data.get("backward") is not None:
            if r < self.size - 1:
                self.h_walls[r+1][c] = sensor_data["backward"]
        
        # Update vertical walls based on left/right sensors
        if sensor_data.get("left") is not None:
            if c > 0:
                self.v_walls[r][c] = sensor_data["left"]
        
        if sensor_data.get("right") is not None:
            if c < self.size - 1:
                self.v_walls[r][c+1] = sensor_data["right"]
        
        # Update boundary walls (always True)
        if r == 0:
            self.h_walls[0][c] = True  # Top boundary
        if r == self.size - 1:
            self.h_walls[self.size][c] = True  # Bottom boundary
        if c == 0:
            self.v_walls[r][0] = True  # Left boundary
        if c == self.size - 1:
            self.v_walls[r][self.size] = True  # Right boundary
    
    def can_move(self, pos, direction):
        """Check if can move in direction based on known walls"""
        r, c = pos
        
        if direction == 'up':
            if r == 0:
                return False
            wall = self.h_walls[r][c]
            return wall == False if wall is not None else True  # Assume open if unknown
        
        if direction == 'down':
            if r == self.size - 1:
                return False
            wall = self.h_walls[r+1][c]
            return wall == False if wall is not None else True
        
        if direction == 'left':
            if c == 0:
                return False
            wall = self.v_walls[r][c]
            return wall == False if wall is not None else True
        
        if direction == 'right':
            if c == self.size - 1:
                return False
            wall = self.v_walls[r][c+1]
            return wall == False if wall is not None else True
...
def visualize(maze, robot):
    ...
    def draw(frame):
        ...
        if frame < len(robot.path):
            ...
            # Title
            title = f'Visited: {current_count}/25 | Yellow=Sensing | Blue=Sensed'
...
def main():
    ...
    # Set starting position (where drone currently is)
    start_pos = (0, 0)  # Adjust based on where drone starts
    target_pos = (2, 1)  # Adjust based on desired target
    maze.set_start_target(start_pos, target_pos)
