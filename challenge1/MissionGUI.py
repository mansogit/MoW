...
class MissionControlGUI:
    ...
    def create_input_frame(self):
        ...
        # Note about auto-generated start/target
        ttk.Label(input_frame, text="Start & Target:", 
                 font=("Arial", 9, "italic")).grid(row=1, column=0, sticky=tk.W, pady=5)
        ttk.Label(input_frame, text="Auto-generated by maze", 
                 font=("Arial", 9, "italic"), foreground="gray").grid(row=1, column=1, columnspan=2, sticky=tk.W, pady=5, padx=5)
        
        # Visualization speed
        ttk.Label(input_frame, text="Visualization Delay (ms):").grid(row=2, column=0, sticky=tk.W, pady=5)
    ...
    def run_mission(self, maze_size):
        ...
        try:
            ...
            print(f"Maze generated: Start={maze.start}, Target={maze.target}")
            
            # Create robot with custom exploration that updates GUI
            ...
            print(f"{'='*60}\n")
            print("Visualization complete. Data saved to 'maze_data.json'.\n")
    ...
    def explore_with_visualization(self, robot):
        ...
        print(f"  Target position: {maze.target}")
        
        # Phase 2: Find shortest path
        print("\n🔍 Computing shortest path from current position to target...")
        robot._find_shortest_path()
        print(f"✓ Shortest path found: {len(robot.shortest_path)} steps")
        
        # Phase 3: Navigate to target using shortest path
        print("\n🎯 Moving to target via shortest path...")
        for next_pos in robot.shortest_path[1:]:
            r1, c1 = robot.pos
            r2, c2 = next_pos
            if r2 < r1: robot.move('up')
            elif r2 > r1: robot.move('down')
            ...
            # Update GUI with shortest path visualization
            self.root.after(0, lambda idx=frame_idx: self.draw_maze_state(maze, robot, idx))
            time.sleep(self.visualization_delay * 2)  # Slower for shortest path
