...
def explore_maze_with_backtracking(start_r, start_c, maze):
    ...
    while stack:
        ...
        if neighbors:
            ...
            stack.append((nr, nc))
            visited.add((nr, nc))
            path.append((nr, nc))  # one-step move
        else:
            stack.pop()
            if stack:
                back_r, back_c = stack[-1]
                path.append((back_r, back_c))  # one-step backtrack
...
# 1. Explore entire maze (with backtracking)
exploration_path, visited_cells = explore_maze_with_backtracking(start[0], start[1], maze)

# 2. Ensure ALL cells were explored before going to target
if len(visited_cells) != ROWS * COLS:
    raise RuntimeError("Robot did NOT explore all cells before heading to target!")

# 3. Only AFTER full exploration, move to target
...
# 4. Final robot path (exploration + goal)
