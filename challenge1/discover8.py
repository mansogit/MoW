...
class Robot:
    ...
    def sense(self):
        """Sense walls around current position"""
        return {d: self.maze.can_move(self.pos, d) 
            ...
            for r in range(6):
                for c in range(5):
                    if self.maze.h_walls[r][c]:
                        ax.plot([c*30, c*30+30], [r*30, r*30], 'k-', linewidth=3)
            for r in range(5):
                for c in range(6):
                    if self.maze.v_walls[r][c]:
                        ax.plot([c*30, c*30], [r*30, r*30+30], 'k-', linewidth=3)
