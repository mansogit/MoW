...
class MissionControlGUI:
    ...
    def run_race(self):
        ...
        try:
            ...
            # Optimize path by grouping consecutive moves in same direction
            optimized_path = self.optimize_path_with_leaps(shortest_path)
            print(f"Optimized to {len(optimized_path)} movements (grouped consecutive steps)")
            print(f"Starting navigation...\n")
            
            # Animate the race
            self.animate_race(optimized_path, start_position, target_position, sensed_barriers, size)
    ...
    def optimize_path_with_leaps(self, path):
        """Optimize path by grouping consecutive moves in same direction into leaps"""
        if len(path) <= 1:
            return [{'pos': path[0], 'steps': 1, 'direction': 'start'}] if path else []
        ...
        while i < len(path):
            ...
            # Determine direction from previous position
            prev_pos = path[i-1]
            dr = current_pos[0] - prev_pos[0]
            dc = current_pos[1] - prev_pos[1]
            
            if dr == -1: direction = 'up'
            elif dr == 1: direction = 'down'
            ...
            while j < len(path):
                next_pos = path[j]
                prev_check = path[j-1]
                dr_next = next_pos[0] - prev_check[0]
                dc_next = next_pos[1] - prev_check[1]
                
                if dr_next == -1: next_dir = 'up'
                elif dr_next == 1: next_dir = 'down'
                elif dc_next == -1: next_dir = 'left'
                elif dc_next == 1: next_dir = 'right'
                else: next_dir = 'unknown'
            ...
            # Add the leap
            optimized.append({
                'pos': path[i + steps - 1],  # End position of the leap
                'steps': steps,
                'direction': direction,
                'start_pos': current_pos,
                'end_pos': path[i + steps - 1]
            })
    ...
    def animate_race(self, optimized_path, start, target, sensed_barriers, size):
        """Animate the race path on the GUI with optimized leaps"""
        print(f"Animating {len(optimized_path)} movements...")
        for i, move in enumerate(optimized_path):
            ...
            if steps > 1:
                print(f"  Movement {i+1}/{len(optimized_path)}: {direction.upper()} x{steps} blocks to {pos}")
            else:
                print(f"  Movement {i+1}/{len(optimized_path)}: {pos} ({direction})")
            
            # Schedule GUI update on main thread
            self.root.after(0, lambda idx=i: self.draw_race_state_optimized(optimized_path, idx, start, target, sensed_barriers, size))
            time.sleep(self.visualization_delay * 2)
    
    def draw_race_state_optimized(self, optimized_path, current_idx, start, target, sensed_barriers, size):
        """Draw the race state with optimized path showing leaps"""
        self.ax.clear()
        ...
        # Draw completed movements
        for i in range(current_idx):
            move = optimized_path[i]
            pos = move['pos']
            r, c = pos
            if (r, c) != start and (r, c) != target:
                # Draw marker at end of each leap
                circle = patches.Circle((c*30+15, r*30+15), 6,
                                       facecolor='lime', edgecolor='darkgreen', linewidth=2)
                self.ax.add_patch(circle)
                
                # If it was a multi-step leap, show the leap path
                if move['steps'] > 1 and 'start_pos' in move:
                    sr, sc = move['start_pos']
                    # Draw arrow showing the leap
                    self.ax.annotate('', xy=(c*30+15, r*30+15), xytext=(sc*30+15, sr*30+15),
                                   arrowprops=dict(arrowstyle='->', color='lime', lw=2, alpha=0.7))
                    # Add text showing number of steps
                    mid_r = (sr + r) / 2
                    mid_c = (sc + c) / 2
                    self.ax.text(mid_c*30+15, mid_r*30+15, f'x{move["steps"]}', 
                               ha='center', va='center', fontsize=9, weight='bold',
                               color='white', bbox=dict(boxstyle='round,pad=0.3', 
                               facecolor='green', alpha=0.8))
        
        # Draw full path as dashed line
        for i in range(len(optimized_path) - 1):
            move1 = optimized_path[i]
            move2 = optimized_path[i + 1]
            r1, c1 = move1['pos']
            r2, c2 = move2['pos']
            self.ax.plot([c1*30+15, c2*30+15], [r1*30+15, r2*30+15], 
                        'lime', linewidth=2, linestyle='--', alpha=0.3)
        ...
        if current_idx < len(optimized_path):
            current_move = optimized_path[current_idx]
            r, c = current_move['pos']
            ...
            # Show current movement info
            if current_move['steps'] > 1:
                # Draw arrow for current leap
                if 'start_pos' in current_move:
                    sr, sc = current_move['start_pos']
                    self.ax.annotate('', xy=(c*30+15, r*30+15), xytext=(sc*30+15, sr*30+15),
                                   arrowprops=dict(arrowstyle='->', color='orange', lw=3, alpha=0.9))
        
        # Title with leap information
        total_movements = len(optimized_path)
        progress = (current_idx / (total_movements - 1) * 100) if total_movements > 1 else 100
        movements_remaining = total_movements - 1 - current_idx
        
        current_move = optimized_path[current_idx] if current_idx < len(optimized_path) else optimized_path[-1]
        title = f'RACE - Optimized Path (Grouped Consecutive Moves)\n'
        title += f'Movement {current_idx + 1}/{total_movements}'
        
        if current_move['steps'] > 1:
            title += f' | LEAP: {current_move["direction"].upper()} x{current_move["steps"]} blocks'
        else:
            title += f' | {current_move["direction"].upper()}'
