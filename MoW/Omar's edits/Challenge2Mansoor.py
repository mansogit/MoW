...
class Challenge2GUI:
    ...
    def create_gui(self):
        ...
        ttk.Label(mapping_frame, text="Maps entire maze", font=('Arial', 8, 'italic')).grid(row=0, column=0, columnspan=2)
        ...
        ttk.Button(mapping_frame, text="Load Saved Map", command=self.reload_map).grid(row=2, column=0, columnspan=2, pady=2, sticky="ew")
        ...
        ttk.Label(race_targets_frame, text="Add targets for race:", font=('Arial', 8, 'italic')).grid(row=0, column=0, columnspan=2)
        ...
        target_facing_combo = ttk.Combobox(race_targets_frame, textvariable=self.target_facing, width=8, state="readonly")
        target_facing_combo['values'] = ('North', 'East', 'South', 'West')
        target_facing_combo.current(0)
        target_facing_combo.grid(row=3, column=1)

        ttk.Button(race_targets_frame, text="Add Target", command=self.add_race_target).grid(row=4, column=0, columnspan=2, pady=2)
        ttk.Button(race_targets_frame, text="Clear Targets", command=self.clear_race_targets).grid(row=5, column=0, columnspan=2, pady=2)

        # Targets listbox
        self.race_targets_listbox = tk.Listbox(race_targets_frame, height=4, width=23)  # Reduced from height=5, width=25
        ...
        ttk.Checkbutton(racing_frame, text="Object Detection", variable=self.enable_detection).grid(row=3, column=0, columnspan=2, sticky="w")
    ...
    def detect_object_at_target(self, target_num):
        ...
        # Get current position for altitude maintenance
        # Note: This requires the current position to be passed in
        # For now, we'll maintain altitude by not sending any movement commands
        ...
        try:
            ...
            while time.time() - start_time < hover_time:  # Configurable detection window
                try:
                    ...
                    if frame_count == 1 or (frame_count < 5):
                        self.log(f"[DEBUG] Got frame {frame_count}, shape: {frame.shape}")
                    
                    # Get detection result with confidence
                    # The onnxdetector returns (object_name, annotated_frame)
            ...
            if detected_obj:
                ...
                if detected_frame is not None:
                    self.root.after(0, lambda: self.display_detection_image(
                        detected_frame, detected_obj, detected_confidence, target_num
                    ))
            else:
    ...
    def run_complete_mapping(self):
        ...
        try:
            ...
            BLOCK_SIZE = 60
            FLIGHT_HEIGHT = 80  # Standard height matching other working scripts
            OFFSET_X = 0
            OFFSET_Y = 0
    ...
    def run_race(self):
        ...
        try:
            ...
            BLOCK_SIZE = 60
            FLIGHT_HEIGHT = 80  # Standard height matching mapping phase
